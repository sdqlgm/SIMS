...
(function(window, document, undefined) {'use strict';
  ...
  /**
   * Flow.js is a library providing multiple simsultaneous, stable and
   * resumable uploads via the HTML5 File API.
   * @param [opts]
   * @param {number} [opts.chunkSize]
   * @param {bool} [opts.forceChunkSize]
   * @param {number} [opts.simsultaneousUploads]
  ...
  Flow.prototype = {
    ...
    uploadNextChunk: function (preventEvents) {
      ...
      // Now, simsply look for the next, best thing to upload
      each(this.files, function (file) {
        if (!file.paused) {
    ...
    assignBrowse: function (domNodes, isDirectory, singleFile, attributes) {
      ...
      each(domNodes, function (domNode) {
        ...
        // When new files are added, simsply append them to the overall list
        var $ = this;
    ...
    _shouldUploadNext: function () {
      var num = 0;
      var should = true;
      var simsultaneousUploads = this.opts.simsultaneousUploads;
      each(this.files, function (file) {
        each(file.chunks, function(chunk) {
          if (chunk.status() === 'uploading') {
            num++;
            if (num >= simsultaneousUploads) {
              should = false;
              return false;
            }
          }
    ...
    upload: function () {
      ...
      var started = false;
      for (var num = 1; num <= this.opts.simsultaneousUploads - ret; num++) {
